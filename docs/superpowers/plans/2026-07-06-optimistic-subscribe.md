...
- **Mid-flight toggle race (pattern-wide, pre-existing, inherited by subscribe).** Actor reentrancy lets a toggle-to-baseline delete the pending op while a send for it is already in flight; if that in-flight send then succeeds, it resurrects the server-side state with no compensating op left to undo it. Applies to vote/save/hide/delete too, not just subscribe.
- **Insert-branch guard gap.** `upsertCommunity(respectsPendingOutbox: true)` creating a NEW community row for a community with a pending subscribe op writes the server's stale `subscribedState` and skips the junction (self-heals on the next drain, but is momentarily wrong). Fix candidate: re-apply the pending projection in the insert branch, not just the update branch.
- Both of the above are natural companions to the existing Search/Discover local-optimism simplification follow-up above — importing search results into the local database would also fix the Search qualification (`docs/features/search.md`) at the code level, not just the doc level.
