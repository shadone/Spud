...
**Env verdict (RESOLVED 2026-07-06 evening, before dispatch): GREEN.** The "host snapshot drift" of spec §5 was actually the shared sim's **Dynamic Type content size sitting at `medium`** (one notch below the iOS default `large`) — sim state, surviving Mac reboots, shrinking every text-bearing ref ~4% while geometry-only refs passed. Diagnosed via failure-diff metrics + `xcrun simctl ui <sim> content_size`; restored to `large` → probe classes 19/19 green. Therefore:
- Record the pixel refs normally (focused run records+fails, rerun verifies; stage ONLY the new refs by explicit path).
- ALSO include the durable defense: pin `preferredContentSizeCategory: .large` into the shared snapshot determinism traits (`SpudSnapshotTests/SnapshotDeterminism.swift` / the `deterministicPhone` config — read how traits are built there and add the category the same way `pinAccent` pins accent), so the suite is immune to future sim-level Dynamic Type pollution. This must be byte-identical for all existing refs (the sim is now at the default the refs were recorded under) — verify with a full focused-classes pass; if ANY existing ref changes under the pin, stop and report rather than re-recording.
- Keep the non-pixel geometry test as well (mirroring `PersonContentUnavailableSnapshotTests.test_errorState_contentDoesNotOverlapHeader`): header taller than half the table height, assert the `UIContentUnavailableView` frame sits entirely below the header's maxY — it guards the inset independent of pixel environment.
...
- [ ] Env-resolution docs (from this branch's investigation): `SpudSnapshotTests/CLAUDE.md` — add the sim-Dynamic-Type gotcha next to the stale-app one (text-metric ref failures suite-wide, reboot-immune; diagnose `xcrun simctl ui <sim> content_size` [underscore], restore `large`; now also pinned in the determinism traits per Task 2); `docs/superpowers/specs/2026-07-05-follow-ups.md` §5 — append a dated addendum resolving the "host drift" escalation (root cause: sim content size at `medium`; suite re-verified 258/258 on 2026-07-06 late evening; snapshot verification UNBLOCKED).
