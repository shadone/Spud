...
- A `capability.blocked` event (category `.site`, level `.info`) is recorded whenever an operation is blocked because the account's home instance doesn't support it yet (e.g. a Lemmy 1.0 instance's transitional API) — both when a gated `LemmyService` call throws its typed `unsupportedByInstance` error, and when a silent soft-degrade (unread count, NSFW/default-sort preference push, read-state sync) skips its server round-trip with no user-visible change. Metadata carries the blocked `capability` name; info level, not error, since this is an expected, version-gated condition rather than a failure. See [Instance capability gating](instance-capability-gating.md).
- Reminder events (category `reminder`, see [reminders.md](reminders.md)) cover both the activity-follow poll and its background trigger. The poll itself (foreground tick or background wake-up alike) records `poll.fired` (`.info`, metadata `postServerId`/`newComments`) when a follow's smart rule trips, and `poll.fetchFailed` (`.notice`, metadata `postServerId`) when it couldn't refresh a target's comment count that round. The `BGAppRefreshTask` wrapper around it separately records `bg.scheduled`/`bg.scheduleFailed` (`.debug`/`.notice`) when the next background request is submitted, and `bg.ran`/`bg.expired` (`.debug`/`.notice`) for whether a launched background run completed or was cut off by the OS budget.
