...
- **Avatar.** Tapping the circular avatar area opens the system photo picker and compresses to JPEG (the same encode path as [Image upload](image-upload.md)); an upload spinner shows while the preview is prepared. A long-press or context menu offers Remove, which clears the avatar. A missing avatar falls back to a deterministic hue tile.
...
- **Avatar and banner are pushed to the server on Save.** A newly-picked avatar or banner is uploaded to your instance when you tap Save, and removing either clears it server-side — so the change follows your account across devices, not just on this device. Newer instances (Lemmy 1.0) use dedicated profile-image endpoints; older instances upload to pict-rs and store the resulting URL. Upload/preview errors surface inline without clearing the field, so you can retry. Removing the banner or avatar offline behaves like any profile save offline — the existing error path applies (no new offline semantics).
