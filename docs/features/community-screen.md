...
- **Empty and error states also sit below the header.** If the community has no posts, or the feed's first load fails, the same below-header placement applies: the full empty or error placeholder is inset to start below the header instead of centering across the whole screen, so it never renders underneath the header either. See [Feed loading and pagination](feed-loading.md).
