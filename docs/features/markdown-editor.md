...
- **Live preview rendering.** Entering Preview re-renders the current draft through `MarkdownBodyView` and `MarkdownBlockCache` — the same `SpudMarkdownKit` pipeline used by post detail and comments — so the preview matches the final rendered result exactly. Links in the preview are tappable; an empty draft shows a "Nothing to preview" placeholder.
