...
| [PieFed instances](piefed.md) | `iphone`, `ipad` | partial — browse-only (Phase 1): reading a PieFed instance's feeds, communities, posts + comments, and search works (routed through a PieFed `/api/alpha` dialect selected from NodeInfo detection, decoded into the same neutral content types as Lemmy; opening a person profile — from an author tap or a Search Users-scope result — doesn't, that's Phase 2 too); login and every write/interaction (vote/save/subscribe/comment/post/DM/inbox) are Phase 2, and login is still blocked by the software-detection pre-flight |
...
- [~] PieFed instances (browse-only, Phase 1) — a PieFed instance (detected via NodeInfo) is browsable signed-out: its feeds, communities, posts + comment trees, and search all render, decoded into the same neutral content types as Lemmy by a dedicated PieFed `/api/alpha` dialect chosen from the detected software (never from a Lemmy-scale version parse); opening a person profile (from an author tap or a Search Users-scope result) does not — that's Phase 2 as well; no account needed; logging in and every write/interaction (vote/save/subscribe/hide/comment/post/report/moderation/DM/inbox/notifications) are Phase 2, login remains blocked by the software-detection pre-flight, and PieFed-native extras (reactions/flair/topics/feeds) decode harmlessly but are unsurfaced (Phase 3) (piefed.md)
