...
| [Share as Image](share-as-image.md) | `iphone`, `ipad` | shipped — designed post/comment-chain card (fixed theme-independent palette), redact-identities, spoilered NSFW with per-card reveal, light/dark + native/square/story canvas, direct-manipulation editor with last-used settings; entry points mirror Share (feed/Search menu, post-detail overflow/header/comment menus) |
| [Media viewer and inline video](media-viewer.md) | `iphone`, `ipad` | shipped — incl. "Showing low-resolution preview" pill when the full-res image can't load over a thumbnail; recognized video hosts (streamable + PeerTube + loops.video + YouTube family incl. Piped/Invidious front-ends) play inline with browser fallback on resolution failure; YouTube-family inline requires a Piped front-end configured in Privacy settings or the opt-in "Play YouTube Videos Inline" toggle (default off) |
...
| [Community screen](community-screen.md) | `iphone`, `ipad` | shipped — overflow: subscribe, favorite, "Notify About New Posts", mute, block, share; on iPad (regular width) opens as a two-column reading split (see [iPad split-view handoff](ipad-split-view.md)) |
| [Instance meta communities](instance-meta-communities.md) | `iphone`, `ipad` | shipped — heuristic "Instance community" badge on Discover / Communities tab / Search / community header; always-visible "About `<instance>`" section in the Communities tab (high-confidence first, low-confidence under a disclosure) with one-tap Favourite (always) / a notify bell for new posts (always, see [Reminders](reminders.md)) / Subscribe (signed in); a dedicated meta section on the instance-detail screen is not shipped |
...
| [Instance capability gating](instance-capability-gating.md) | `iphone`, `ipad` | shipped, **live for PieFed** (withholds image upload + server-settings push, the first real gap set) and dormant for Lemmy — gates nothing on any Lemmy version now that Spud speaks native v4; unavailable features present via the gate sheet or the non-retriable "Not available on this instance" surface; still fails open on unknown software/version |
...
| [Reminders](reminders.md) | `iphone`, `ipad` | shipped — time-based "Remind Me…" reminders (local OS notification + Inbox Reminders segment + tab badge); "When there are new comments" activity follow, checked via a foreground poll plus a best-effort `BGAppRefreshTask` background poll; either can target the whole post OR a single comment's thread (from the comment's context menu), independently; a thread reminder opens the post scrolled to that comment; a third kind, "Notify About New Posts", follows a whole community instead (5 entry points: community overflow/header menu, Search, a meta-row bell, and the Communities tab's subscribed-list menu), fires on any ≥1 new post (batched, "N+" when the fetch saturates), pauses while the community is muted and fires the backlog as one batch on unmute, and is removed when the community is blocked; removing/logging out of an account cancels its reminders and follows |
...
- [x] Share as Image — "Share as Image" next to Share on the feed/Search post menu, post-detail overflow menu, post-detail header menu, and a comment's context menu; renders a fixed-palette, theme-independent card (post, or a comment plus its upward ancestor chain, eliding a deep chain's middle) with toggleable community/creator, media (NSFW spoilered by default, per-card reveal never persisted), body treatment (full/truncate-with-fade/title-only), stats, identity redaction (community never redacted), and native/square/story canvas; the permalink and an absolute timestamp render in every configuration; a direct-manipulation editor (tap card elements to toggle them) opens with the last-used configuration and outputs via Share (alt text embedded in the PNG), Save to Photos, or Copy (share-as-image.md)
...
- [x] Animated GIF playback; inline video — direct mp4/mov/m4v plays via the system player; recognized video hosts (streamable.com, PeerTube instances, loops.video, and the YouTube family — youtube.com/youtu.be, Piped, Invidious front-ends, and the bare `/watch?v=` shape) resolve to their stream and play inline, with browser fallback on resolution failure; YouTube-family inline requires a Piped front-end in Privacy settings or the opt-in "Play YouTube Videos Inline" toggle, which falls back to the default cataloged Piped instance (otherwise the original page opens in the browser; Google is never contacted for playback)
