...
| [Search](search.md) | `iphone`, `ipad` | shipped — scopes: posts / communities / users / comments (federated) + instances (local directory); post results reuse the rich feed cell (community@instance, counts, thumbnail, status/author badges) + an author line, vote arrows suppressed; NSFW posts render blurred (not dropped); post/community/comment/user results each carry a long-press context menu (feed parity / Discover parity / post-detail comment-menu parity / Person-profile-header-menu parity respectively; instance results don't yet) |
...
- [x] Search (posts / comments / communities / users federated, + instances over the local Explorer directory) — post results render as the shared rich feed cell (community@instance, counts, thumbnail, status/author badges) plus a `@user@instance` author line, with vote arrows suppressed and NSFW posts blurred rather than dropped; long-press a post result for the same context menu the feed uses (vote/save/reply/share/cross-post/visit community/view author/hide/block/report/mute/remind me); long-press a community result for Discover's community context menu (open/subscribe/mute/share/copy link/block); long-press a comment result for post detail's comment context menu (open thread/vote/save/share/copy link/view author/report); long-press a user result for the Person profile's header context menu (open profile/copy handle/share/block user); + inline subscribe + paste-a-Lemmy-URL "Open in Spud" (canonical + frontend `/c/../p/<id>` form)
