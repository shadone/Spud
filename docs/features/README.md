...
| [Account Activity](account-activity.md) | `iphone`, `ipad` | shipped (Phase 1) — reverse-chronological timeline of everything the signed-in account has done (authored posts + comments, upvoted/downvoted, saved, read, seen, hidden); filter chip bar (7 types); search; day-based grouping; infinite scroll; preset filters from Account tab shortcuts |
...
- [x] Account Activity — reverse-chronological timeline of posts/comments authored, upvoted/downvoted, saved, read, seen, hidden; 7-type filter chip bar with preset filters from Account tab shortcuts (Saved/Your posts/Your comments open it pre-filtered); search; day-based grouping; infinite scroll (account-activity.md)
