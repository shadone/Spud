...
| [Search](search.md) | `iphone`, `ipad` | shipped — scopes: posts / communities / users / comments (federated) + instances (local directory); post results reuse the rich feed cell (community@instance, counts, thumbnail, status/author badges) + an author line, vote arrows suppressed; NSFW posts render blurred (not dropped); post results carry the full feed long-press context menu (community/user/comment/instance results don't yet) |
...
- [x] Search (posts / comments / communities / users federated, + instances over the local Explorer directory) — post results render as the shared rich feed cell (community@instance, counts, thumbnail, status/author badges) plus a `@user@instance` author line, with vote arrows suppressed and NSFW posts blurred rather than dropped; long-press a post result for the same context menu the feed uses (vote/save/reply/share/cross-post/visit community/view author/hide/block/report/mute/remind me); + inline subscribe + paste-a-Lemmy-URL "Open in Spud" (canonical + frontend `/c/../p/<id>` form)
