...
| [Reminders](reminders.md) | `iphone`, `ipad` | partial — Phase 1: time-based "Remind Me…" reminders on a whole post (local OS notification + Inbox Reminders segment + tab badge). Phase 2: "When there are new comments" whole-post activity follow, checked via a foreground poll (no background task yet), surfaced in the same Reminders segment; comment-subtree targeting is a later phase |
...
- [~] Reminders — "Remind Me…" time presets (whole post) from the post-detail overflow menu and feed context menu; delivered as a local OS notification (or in-app only if notifications are denied) and surfaced in a dedicated Inbox Reminders segment with its own fired-unseen tab-badge contribution; local/durable only, no backend. Same menu's "When there are new comments" toggle follows a whole post's discussion, checked by a foreground poll (smart rule: ≥5 new comments, or ≥1 new after 24h) and shown in the same Reminders segment ("Watching for new comments" / "New comments · tap to catch up"); works signed-out too. Comment-subtree targeting and background (`BGAppRefreshTask`) polling are later phases (reminders.md)
