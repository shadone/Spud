...
| [Search](search.md) | `iphone`, `ipad` | shipped — scopes: posts / communities / users / comments (federated) + instances (local directory); post results reuse the rich feed cell (community@instance, counts, thumbnail, status/author badges) + an author line, vote arrows suppressed; NSFW posts render blurred (not dropped); post/community/comment results each carry a long-press context menu (feed parity / Discover parity / post-detail comment-menu parity respectively; user/instance results don't yet) |
