...
A single recovery screen lists every pending or failed outbound content item — unsent drafts, in-flight sends, and permanently-failed items — for the signed-in account. It is the one place to retry failed sends, review what is queued, and discard items that should not be sent. Reachable from a row on the Account tab (signed-in accounts only) and from the "Couldn't post / Couldn't send comment — View" failure toast.
...
- **Account tab entry point.** A "Drafts & Outbox" row on the signed-in Account tab opens this screen at any time, not only after a failure. It is per-account content, so it lives with the account's other shortcuts (Saved, Activity, Your posts, Your comments) rather than in app Settings. Signed-out (anonymous) accounts have no outbound content and so do not show the row.
...
- **When** I open Drafts and Outbox (via the toast or the Account tab) and swipe Retry
