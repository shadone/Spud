...
- **A "Top of …" default keeps its time window.** For a time-scoped default such as Top-of-Week, the window is preserved end to end: the push sends the exact window (Top-Week stays Top-Week rather than collapsing to Top-of-all-time), and the import reads it back with the same window. This holds on both older instances and Lemmy 1.0 (which model the window differently under the hood).
