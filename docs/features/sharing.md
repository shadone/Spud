...
- **Canonical URL preferred.** The shared URL prefers the content's federation permalink (its `ap_id`), falling back to your account's home instance as `<instance>/post/<id>` or `<instance>/comment/<id>`. You can change this behavior in Settings → General → Post & Comment Links to always use your home instance. If no URL can be formed, a warning haptic fires and nothing is shared. See [External link handling](external-link-handling.md) for the related per-link preference.
