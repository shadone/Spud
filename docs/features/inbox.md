...
- **Replies and mentions are comment rows.** Each shows the author, the comment body, and an "in `<community>` · `<post title>`" context line. Tapping a row opens that comment's post in the post detail (and marks the row read — see [Mark inbox items read](inbox-mark-read.md)).
- **Messages is a conversation list.** Private messages are grouped per correspondent into conversations, newest thread first, each row showing the correspondent's name, avatar, and the latest message as a preview. Tapping a row opens the DM thread (see [Private messages](private-messages.md)).
...
- **Pull-to-refresh.** The Inbox list has a pull-to-refresh control that reloads all three scopes and the unread count. (This is unlike the main post feed, which has no pull-to-refresh.)
