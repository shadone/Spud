...
- **Designed states.** Each tab shows a spinner while loading, an empty state ("No posts" / "No comments") when that tab is empty, and an error state with a pull-to-refresh hint on failure. These states render inside the content area below the always-present header (avatar / name / Posts|Comments switcher), not as a view-wide overlay, so the message never covers the header and pull-to-refresh stays available.
