...
- **A "Downloaded" entry appears when you have offline downloads.** The switcher shows a "Downloaded" feed (after Saved) whenever there are posts saved for offline reading, and hides it otherwise — its presence is re-evaluated each time the switcher appears. The Downloaded feed is local-only (it never fetches from the network); it is documented in [Download a feed for offline browsing](offline-download.md).
