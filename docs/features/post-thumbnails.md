...
- **Content type drives the thumbnail.** Spud classifies a post's URL into image, video, external link, or text/empty, and picks the thumbnail accordingly. Image detection is by file extension (`.jpg`, `.jpeg`, `.png`, `.webp`, `.gif`); a post is video either by a playable file extension (`.mp4`, `.mov`, `.m4v`) or by being a recognized video host (streamable.com, a PeerTube instance, or a YouTube-family link — youtube.com/youtu.be, Piped, Invidious front-ends, and the bare `/watch?v=<id>` shape on unknown hosts). A recognized-host video shows the server thumbnail as its poster with the play indicator, just like a direct video post. YouTube-family posts (including Invidious front-end links) always show the video badge, but inline playback happens only when the user's YouTube front-end is a Piped instance or the opt-in "Play YouTube Videos Inline" setting is on (otherwise tapping opens the original page in the browser).
