...
- **Content type drives the thumbnail.** Spud classifies a post's URL into image, video, external link, or text/empty, and picks the thumbnail accordingly. Image detection is by file extension (`.jpg`, `.jpeg`, `.png`, `.webp`, `.gif`); a post is video either by a playable file extension (`.mp4`, `.mov`, `.m4v`) or by being a recognized video host (streamable.com or a PeerTube instance). A recognized-host video shows the server thumbnail as its poster with the play indicator, just like a direct video post.
