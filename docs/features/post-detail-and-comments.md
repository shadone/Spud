...
- **Attribution shows full handles.** The attribution reads "in `<Community>`@`<instance>` by `<Author>`@`<instance>`": the community's display name and the author's display name carry the tap targets (to the community and the person profile), each followed by its home instance host in a muted style — matching how instance hosts are dimmed in the post-list rows. A purely local handle still shows its own instance host.
