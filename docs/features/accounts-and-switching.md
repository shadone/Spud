...
- **Status:** shipped
- **Related:** [Edit your profile](profile-editing.md), [Account Activity](account-activity.md), [Person / user profile](person-profile.md), [Saving](saving.md), [Marking posts read and hiding read posts](mark-read-and-hiding.md), [Signed-out browsing](signed-out-browsing.md), [Login](login.md), [Instance picker](instance-picker.md), [Registration](registration.md), [Sign-in gate on write actions](sign-in-gate.md), [Session re-login hint](session-reauth.md), [DESIGN-BRIEF.md](../design/DESIGN-BRIEF.md)
...
- **A stored session can expire.** If an account's server-side session is rejected as expired or revoked, it is flagged and shown a quiet "Session expired" hint — an Account-tab badge, a switcher row affordance, an Account-screen row, and a blocked-action toast — all leading to an in-place re-login for that same account. See [Session re-login hint](session-reauth.md).
