...
The Account tab itself is the signed-in account's home: a tappable profile header — full-width banner (if set) with the circular avatar overlapping its bottom edge, plus the display name and `@name@instance` handle — which opens [Edit your profile](profile-editing.md), over a grouped list of shortcuts — Switch account, Saved, Activity, Your posts, Your comments, and Log out — with Settings in the nav bar.
