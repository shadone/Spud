...
open class LinkLabel: UILabel, UIGestureRecognizerDelegate {
    ...
    open var linkTextAttributes: [NSAttributedString.Key: AnyObject] {
        didSet {
    ...
    open var highlightedLinkTextAttributes: [NSAttributedString.Key: AnyObject] {
        didSet {
            self.setupAttributes()
        }
    }
            ...
            if newValue != nil {
                ...
                self.attributedText!.enumerateAttributes(in: range, options: [], using: { (attributes, range, _) in
                    for attribute in attributes {
                        if attribute.key == NSAttributedString.Key.link {
