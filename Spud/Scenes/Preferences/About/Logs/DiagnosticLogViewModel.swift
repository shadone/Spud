...
final class DiagnosticLogViewModel {
    ...
    /// Produces a plain-text export of the current `events` list — one
    /// ``DiagnosticEventText/line(_:)`` per event, joined with newlines,
    /// newest-first (same order as the displayed list).
    func shareText() -> String {
        events.map(DiagnosticEventText.line).joined(separator: "\n")
    }
