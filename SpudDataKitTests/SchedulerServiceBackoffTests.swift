...
struct SchedulerServiceBackoffTests {
    ...
    func backoff_gatesRepeatedFetchesAndResetsOnReconnect() async throws {
        // Arrange: in-memory DB seeded with one signed-out account awaiting site info.
        // The account appears in `signedOutAccountsAwaitingSiteInfo()` because
        // its joined site row has name IS NULL (the default for a freshly inserted site).
