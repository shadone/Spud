...
- **SpudDataKit now consumes LemmyKit's version-neutral `*Neutral` API** (dual v3/v4, v4 semantics — see LemmyKit's CLAUDE.md), so UITest stubs must match the NEUTRAL wire requests, not the raw v3 endpoints. What changed and bit the stubs: community-by-name resolves via `GET /resolve_object?q=<ap_url>` (the `GET /community?name=` endpoint is RETIRED as a Spud request), the community feed requests `community_id=` (not `community_name=`), and `getCommentsNeutral` sends NO `max_depth`. A stale stub/matcher → 500 or decode-throw → the `*OrLoading` VC silently bails and the assertion fails (reads like a UI bug, is a fixture bug). Canonical example: `resolve-object-community-tincidunt.json` + the neutral-shape refresh in `SpudUITests`/`IPadSplitUITests`.
