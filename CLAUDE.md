...
don't edit existing ones. Latest is `v20_postNsfw` (next is v21) — `v19_favoritedCommunity` and `v20_postNsfw` already exist, so the old "next is v19" note was stale.
...
- Bump `CURRENT_PROJECT_VERSION` in `project.yml` before each release (1,2 failed uploads; 3–10 live; next ≥11).
...
- Body-text links (post & comment) render through `SpudMarkdownKit`: `InlineLexer` autolinks bare URLs and Lemmy mention shorthands (`!c@i`, `@u@i`) at parse time, and `InlineAttributedStringBuilder` stamps `.link` attributes. Bare URLs keep their real `http(s)` URL; mentions/communities become synthetic `spud-markdown://mention|community?name=…&instance=…` URLs. At **tap** time the body's `MarkdownBodyDelegate.markdownBody(didTapLink:)` fires; `MarkdownInternalLink.resolve` (`Spud/Utils/MarkdownInternalLink.swift`) translates a `spud-markdown://` link into the app's internal `URL.SpudInternalLink` (decoded by `URL.spud`), and any other URL falls through to `LemmyURLParser.classify` (path-based `/post` `/c`, bare-instance → internal link). Explicit markdown links whose destination is a Lemmy user/community URL (the `[…](https://instance/u/name)` / `/c/name` form Lemmy emits for `@u@i` / `!c@i`) are rewritten to the synthetic `spud-markdown://` URL at render time by `InlineAttributedStringBuilder.lemmyReferenceURL`, so they resolve **in-app** too (not just shorthand mentions) — bypassing `classify`'s known-instance (Explorer-directory) gate that otherwise sent unknown-instance `/u//c` links to Safari. The old `MarkdownRenderer` / `addingAutolinks` path has been retired (the `Down` SPM dependency is gone).
