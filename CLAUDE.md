...
- Body-text links (post & comment): `MarkdownRenderer.attributedString` → `addingAutolinks` (`NSAttributedString+Autolink.swift`) stamps `.link` attributes. Bare URLs stay plain `http(s)`; Lemmy mention shorthands (`!c@i`, `@u@i`) become synthetic `info.ddenis.spud://internal/...` URLs (community / objectAtURL). Decode a `.link` value with `URL.spud`. `LemmyURLParser.classify` (path-based `/post` `/c`, bare-instance → internal link) runs at **tap** time (`PostDetailViewController.linkTapped`/`linkLongPressed`), not at render time.
