...
- `SpudSnapshots.xctestplan` — snapshot tests only; **run on iPhone 17 Pro, iOS 26.3, portrait** (the reference device+runtime), otherwise the app-level reference images won't match (they drift across iOS minor versions). Pin `OS=26.3` in `-destination` — `name=iPhone 17 Pro` alone can resolve to a 26.0 sim and fail every full-screen snapshot
...
- Bump `CURRENT_PROJECT_VERSION` in `project.yml` before each release (1,2 failed uploads; 3–14 live; next ≥15).
